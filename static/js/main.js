...
}).setView([13.8333000, -88.9167000], 8);
    ...
    setTimeout(async () => {
        if (checked) {
            ...
            array.forEach(({depth, lat, lng, mag}) => {
                ...
                marker.bindPopup(`<b>Magnitud: </b>${strMag}<br><b>Profundidad: </b>${strDepth} km`)
                markers.push(marker)
            })
            loader.style.visibility = "hidden"
        }
...
document.querySelector("#tutorial").addEventListener('click', () => {
    swal({
        ...
        text: "En tremor puedes predecir epicentros de dos maneras, por un punto o por una nube de puntos. Puedes cambiar " +
